 ...
 * your projects. It is capable of interfacing with both 3.3V and 5V logic levels.
 *
 * @section dependencies Dependencies
 *
 * This library depends on the <a href="https://github.com/adafruit/Adafruit_BusIO">
 * Adafruit BusIO library</a> for low-level communication, and the standard Wire library
...
Adafruit_XCA9554::Adafruit_XCA9554() : 
  i2c_dev(nullptr), input_port_reg(nullptr),
  output_port_reg(nullptr), polarity_inversion_reg(nullptr),
  config_reg(nullptr) {
}
...
bool Adafruit_XCA9554::begin(uint8_t i2c_addr, TwoWire *wire) {
  ...
  output_port_reg = new Adafruit_BusIO_Register(i2c_dev, XCA9554_REG_OUTPUT_PORT);
  polarity_inversion_reg = new Adafruit_BusIO_Register(i2c_dev, XCA9554_REG_POLARITY_INVERT);
...
bool Adafruit_XCA9554::pinMode(uint8_t pin, uint8_t mode) {
  ...
  Adafruit_BusIO_RegisterBits pinModeBit = Adafruit_BusIO_RegisterBits(config_reg, 1, pin);
...
bool Adafruit_XCA9554::digitalWrite(uint8_t pin, bool val) {
  ...
  // Create a RegisterBits object for the specific pin
  Adafruit_BusIO_RegisterBits outputBit = Adafruit_BusIO_RegisterBits(output_port_reg, 1, pin);
 ...
 * @returns The state of the pin (true for HIGH, false for LOW), or false if an error occurs.
 */
bool Adafruit_XCA9554::digitalRead(uint8_t pin) {
  ...
  // Create a RegisterBits object for the specific pin
  Adafruit_BusIO_RegisterBits inputBit = Adafruit_BusIO_RegisterBits(input_port_reg, 1, pin);
